...
void displaySortedTotals(int [], int[], int[]);
    ...
    while (option != 5){
        switch (getValidOptionInput(option)) {
            ...
            case 4:
                ...
                createCumulativeTotalEachRange(numAppearances, cumulativeTotalEachRange, RangeUpperLimitNum);
                displaySortedTotals(numAppearances, cumulativeTotalEachRange, RangeUpperLimitNum);
                DisplayMenuOption();
                cin >> option;
        }
    }
...
void createCumulativeTotalEachRange(int numAppearances[NUM_AVENGERS], int RangeUpperLimitNum[6], int cumulativeTotalEachRange[6]) {
    ...
    int j = 0;
    int k = 0;
    int max = NUM_AVENGERS - 1;

    for (max; max >= 0; max--) {
        for (i = 0; i < max; i++) {
            if (numAppearances[i] > numAppearances[i + 1]) {
                swap(numAppearances[i], numAppearances[i + 1]);
            }
        }
    }

    for (i = 0; i <= NUM_AVENGERS; i++) {
        for (numAppearances[k]; numAppearances[k] <= RangeUpperLimitNum[i]; numAppearances[k++]) {
            if (numAppearances[k] < RangeUpperLimitNum[i]) {
                cumulativeTotalEachRange[i] = cumulativeTotalEachRange[i] + 1;
            }
            if (numAppearances[k] >= RangeUpperLimitNum[i]){
                RangeUpperLimitNum[++i];
            }
        }
    }
}
    ...
    void displaySortedTotals(int numAppearances[NUM_AVENGERS], int RangeUpperLimitNum[6], int cumulativeTotalEachRange[6]) {
        int x = 0;
        for (x; x < 6; x++) {
            cout << cumulativeTotalEachRange[x] << right << setw(10) << RangeUpperLimitNum[x] << endl;
        }
    }